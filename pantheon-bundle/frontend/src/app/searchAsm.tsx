...
export interface ISearchAsmState {
...
class SearchAsm extends Component<IAppState, ISearchAsmState> {
          ...
          <div>
            ...
            <DataList aria-label="Simple data list" >
              <DataListItem aria-labelledby="simple-item1">
                ...
                <DataListItemRow id="data-rows" key={this.state.results[SearchAsm.KEY_TRANSIENTPATH]}>
                  {
                  ...
                  <DataListItemRow id="data-rows" key={key}>
                    {this.props.userAuthenticated && !this.state.isEmptyResults &&
                      <Checkbox aria-labelledby="width-ex3-check1"
                        className="checkbox"
                        isChecked={data[SearchAsm.KEY_CHECKEDITEM]}
                        checked={data[SearchAsm.KEY_CHECKEDITEM]}
                        aria-label="controlled checkbox example"
                        id={data[SearchAsm.KEY_TRANSIENTPATH]}
                        name={data[SearchAsm.KEY_TRANSIENTPATH]}
  ...
  private handleSelectAll = (checked: boolean, event: FormEvent<HTMLInputElement>) => {
    ...
    this.state.results.map(dataitem => {
      ...
      newResults[newResults.length - 1][SearchAsm.KEY_CHECKEDITEM] = checked
    })
  ...
  private handleDeleteCheckboxChange = (checked: boolean, event: FormEvent<HTMLInputElement>) => {
    ...
    this.state.results.map(data => {
      ...
      if (data[SearchAsm.KEY_TRANSIENTPATH] === event.target[nameStr]) {
        newResults[newResults.length - 1][SearchAsm.KEY_CHECKEDITEM] = checked
      }
    })
  ...
  private buildSearchUrl() {
    let backend = "/pantheon/internal/assemblies.json?"
  ...
  private buildTransientPathArray = () => {
    ...
    this.state.results.map(item => {
      if (item[SearchAsm.KEY_CHECKEDITEM]) {
        tPaths.push(item[SearchAsm.KEY_TRANSIENTPATH])
      }
    })
    return tPaths
  }
