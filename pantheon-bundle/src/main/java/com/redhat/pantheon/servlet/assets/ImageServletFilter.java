...
/**
 * A servlet filter which serves image assets only. Images must be referened using an absolute path comprised of a
 * prefix (see @{link IMAGE_PATH_PREFIX} for the exact value) plus an identifier. In this servlet the identifier is a
 * Base64 encoding of the actual path stored in the JCR repository for the image.
 * </br>
 * </br>
 * So for example, an image path might be:
 * http://my.server/imageassets/L2NvbnRlbnQvcmVwb3NpdG9yaWVzL3BhbnRoZW9uU2FtcGxlUmVwby9pbWFnZXMvdGlnZXIucG5n with
 * /imageassets being the prefix and everything after (exluding the separating slash) being a Base64 encoding of the
 * path: /content/repositories/pantheonSampleRepo/images/tiger.png which resolves absolutely and unambigously to an
 * image in the system.
 *
 * @author Carlos Munoz
 */
