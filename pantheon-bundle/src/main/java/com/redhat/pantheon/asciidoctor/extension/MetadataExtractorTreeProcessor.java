...
/**
 * A tree processor that extracts moduleMetadata from the asciidoc AST and
 * inserts it into a sling resource.
 *
 * <p>
 ...
 * </ul>
 *
 * A current restriction is that when the extracted moduleMetadata contains
 * variable substitutions, the substitutions will not be applied when recording
 * the value in the resource.
 * </p>
 *
 * <p>
 * For more information about how to interpret and extract data from the
 * asciidoctor AST, see:
...
public class MetadataExtractorTreeProcessor extends Treeprocessor {
    ...
    /**
     * Extracts the document's headline from an asciidoc document. The headline is
     * the first second-level header in the document (if one is present).
     */
        ...
        Optional<StructuralNode> headlineBlock = allNodes.stream()
                ...
                .filter(block -> {
                    try {
                        ...
                        // Asciidoctor (the Ruby code) throws certain exceptions when properties are not
                        // available.
    ...
    /**
     * Extracts the document's abstract. The abstract is the first found block with
     * the 'abstract' role.
     */
    private void extractAbstract(List<StructuralNode> allNodes) {
        Optional<StructuralNode> abstractNode = allNodes.stream().filter(block -> {
            try {
    ...
    /**
     * Extract the module type from the asciidoc content. This method looks at the
     * _module-type property defined in the module.
     *
    ...
    private void extractModuleType(Document document) {
        ...
        if (attValue != null) {
            try {
                ModuleType moduleType = ModuleType.valueOf(attValue.toString().toUpperCase());
                mm.moduleType().set(moduleType);
            } catch (IllegalArgumentException e) {
                mm.moduleType().set(null);
                log.warn(
                        "Invalid argument for " + MODULE_TYPE_ATT_NAME + " asciidoc attribute: " + attValue.toString());
            }
        }
    }
     ...
     * This method should only be called once per document processing, as it can be
     * rather expensive.
     *
     * @param document The document which nodes' are being flat mapped
     * @return A stream of nodes making up the given document, in order of
     *         appearance
     */
    private static Stream<StructuralNode> nodeFlatMap(Document document) {
        return document.getBlocks().stream().flatMap(node -> concat(stream(of(node)), node.getBlocks().stream()));
    }
}
